  ...
  return (
    <div className="relative w-full select-none overflow-hidden rounded-2xl">
      ...
      >
        ...
        {/* Gold pins with SVG animate pulse rings (no JS, pure SMIL) */}
        {goldPins.map(({ x, y, delay }) => (
          <g key={`g${x}-${y}`} transform={`translate(${x},${y})`}>
            <circle r="5" fill="none" stroke="#C9A227" strokeWidth="1.2" strokeOpacity="0">
              <animate attributeName="r" from="5" to="18" dur="2s" begin={`${delay}s`} repeatCount="indefinite" />
              <animate attributeName="stroke-opacity" from="0.65" to="0" dur="2s" begin={`${delay}s`} repeatCount="indefinite" />
            </circle>
            <circle r="5" fill="none" stroke="#C9A227" strokeWidth="1.5" strokeOpacity="0">
              <animate attributeName="r" from="5" to="11" dur="2s" begin={`${delay + 0.6}s`} repeatCount="indefinite" />
              <animate attributeName="stroke-opacity" from="0.5" to="0" dur="2s" begin={`${delay + 0.6}s`} repeatCount="indefinite" />
            </circle>
            <path d="M0-12C-3.8-12 -6.5-9.4 -6.5-6.4C-6.5-2.6 0 5 0 5C0 5 6.5-2.6 6.5-6.4C6.5-9.4 3.8-12 0-12Z" fill="#C9A227" />
            ...
            >
              ...
              <div className="flex flex-col items-end ml-2 shrink-0">
                <span className="text-[#C9A227] text-xs font-medium">
                  ★ <span className="text-[#d1e5fb] ml-0.5">{listing.rating}</span>
                </span>
